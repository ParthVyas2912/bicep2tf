...
@description('Resource ID of an existing App Service Plan. If provided, will use this instead of creating a new one.')
param existingAppServicePlanId string = ''

var abbrs = loadJsonContent('./abbreviations.json')
var resourceToken = toLower(uniqueString(subscription().id, environmentName, location))
var tags = { 'azd-env-name': environmentName }
var functionAppName = !empty(apiServiceName) ? apiServiceName : '${abbrs.webSitesFunctions}qsp-${resourceToken}'
...
// Only create if an existing plan is not provided
module appServicePlan 'br/public:avm/res/web/serverfarm:0.1.1' = if (empty(existingAppServicePlanId)) {
  name: 'appserviceplan'
  scope: rg
  params: {
    name: !empty(appServicePlanName) ? appServicePlanName : 'qsp-${abbrs.webServerFarms}${resourceToken}'
    sku: {
      name: 'FC1'
...
// Reference to existing App Service Plan if provided
resource existingPlan 'Microsoft.Web/serverfarms@2023-01-01' existing = if (!empty(existingAppServicePlanId)) {
  name: last(split(existingAppServicePlanId, '/'))
  scope: resourceGroup(split(existingAppServicePlanId, '/')[2], split(existingAppServicePlanId, '/')[4])
}
...
module api './app/api.bicep' = {
  ...
  params: {
    ...
    appServicePlanId: !empty(existingAppServicePlanId) ? existingAppServicePlanId : appServicePlan.outputs.resourceId
    runtimeName: 'python'
    runtimeVersion: '3.12'
...
module storage 'br/public:avm/res/storage/storage-account:0.8.3' = {
  ...
  params: {
    ...
    blobServices: {
      containers: [{name: deploymentStorageContainerName}, {name: 'qsp'}]
    }
